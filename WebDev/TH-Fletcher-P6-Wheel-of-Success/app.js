...
//random number, times the array values, floor rounds down, +1 ensures we always have a number equal to 1 or more
...
function checkWin() {
    if (show.length === correct.length) { //if the letters displayed match the letters of the array, we show...
        ...
        title.textContent = 'Big BALLER!!';
        startButton.textContent = 'Another GO?'; //same button, just different text
    ...
    if (missed === 5) { //if the amount of times they miss is equal to five, code runs, we have a loser on our hands.
        ...
        startButton.textContent = 'REDEMPTION';
...
//listen for the letters to be selected
